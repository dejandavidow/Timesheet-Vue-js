    ...
    <div v-for="category in categories" :key="category.id">
      <div class="accordion accordion-flush border" id="accordionFlushExample">
        <div class="accordion-item">
          ...
          <div
            v-bind:id="'a' + category.id"
            class="accordion-collapse collapse listcolor"
          ...
          >
            <div class="accordion-body">
              <n-form inline :label-width="80">
                ...
                <n-form-item>
                  <n-button
                    @click="handleUpdate(category.id, category)"
                    type="primary"
                  >
                    Update
                  </n-button>
...
export default {
  ...
  methods: {
    ...
    async searchCategories() {
      await fetch(
        `http://127.0.0.1:8000/api/Search?search=${this.searchTerm}`,
        { method: "GET", headers: { "Content-Type": "application/json" } }
      )
        .then((res) => res.json())
        .then((x) => {
          this.categories = x.data;
          this.pageCount = Math.ceil(x.total / 5);
          this.loaded = true;
        });
    },
  },
